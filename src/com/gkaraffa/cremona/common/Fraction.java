...
public class Fraction extends Object implements Comparable<Object> {
	...
	public static Fraction[] createWorkingCommonTermPair(Fraction fractionA,
			Fraction fractionB) {
		Fraction[] returnArray = new Fraction[2];

		int multiple = fractionB.getDenominator() * fractionA.getDenominator();
		int fANum = fractionA.getNumerator()
				* (multiple / fractionA.getDenominator());
		int fBNum = fractionB.getNumerator()
				* (multiple / fractionB.getDenominator());
	...
	public Fraction add(Fraction incrementor) {
		...
		result = new Fraction(
				(fractionArray[0].getWorkingNumerator()
						+ fractionArray[1].getWorkingNumerator()),
	...
	public int compareTo(Object o) {
		...
		Fraction[] fractionArray = Fraction.createWorkingCommonTermPair(this,
				(Fraction) o);

		if (fractionArray[1].getWorkingNumerator() == fractionArray[0]
				.getWorkingNumerator()) {
			return EQUAL;
		}
		else
			if (fractionArray[0].getWorkingNumerator() < fractionArray[1]
					.getWorkingNumerator()) {
				return BEFORE;
			}
			else {
